...
public class Piece {
	...
	public Piece(TPoint[] points) {
		...
		for (int i = 0; i < width; i++) {
            for (TPoint point : body) {
                if (point.x == i) {
                    skirt[i] = Math.min(skirt[i], point.y);
                }
            }
		}
	}
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		for (int i = 0; i < body.length; i++) {
			nextBody[i] = new TPoint(-body[i].y, body[i].x);
			min = Math.min(min, -body[i].y);
		}
		for (int i = 0; i < nextBody.length; i++) {
			nextBody[i] = new TPoint(nextBody[i].x - min, nextBody[i].y);
		}
	...
	*/
	public boolean equals(Object obj) {
		...
		TPoint[] checkBody = other.body;
		if (checkBody.length != body.length) return false;
        for (TPoint point1 : checkBody) {
            boolean found = false;
            for (TPoint point2 : body) {
